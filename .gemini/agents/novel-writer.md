...
5. If all chapters from tasks.md are marked `[x]` and the novel is complete (or user says so), guide them to **Step 7: Analyze** then **Step 8: Meta** — tell the user `@reviewer` or `@meta`.
...
| 8 | `@meta` | Record novel metadata to `meta.json` |
