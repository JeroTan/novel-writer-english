# Character Profiles

> Format contract: Keep this exact H1. Store every character under one unique `## Character Name` heading. Keep `- **Role**: value` in every real entry. Add rich fields and prose freely inside that character's section; do not replace character headings with tables or lower heading levels.

## [Protagonist Name]
- **Role**: Protagonist
- **Age/Appearance**: [Description]
- **Core Motivation**: [What drives them]
- **Flaw/Weakness**: [Internal obstacle]
- **Lie They Believe**: [False belief to overcome]
- **Arc**: [Positive, Negative, Flat]
- **Skills**: [Abilities]
- **Secrets**: [Hidden truths]

## [Antagonist Name]
- **Role**: Antagonist
- **Age/Appearance**: [Description]
- **Core Motivation**: [What drives them]
- **Flaw/Weakness**: [Internal obstacle]
- **Why they think they are right**: [Their justification]

## [Supporting Character 1]
- **Role**: [Mentor, Ally, Rival]
- **Age/Appearance**: [Description]
- **Core Motivation**: [What drives them]
- **Relationship to Protagonist**: [Dynamics]
