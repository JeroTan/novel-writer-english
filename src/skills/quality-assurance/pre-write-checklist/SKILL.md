...
2. **Verify Specification:** Have you read `specification.md`, or `specification/_main.md` plus relevant shard(s) when split mode exists? What are the core requirements?
3. **Verify Plan:** Have you read `creative-plan.md`, or `creative-plan/_main.md` plus the exact chapter shard when split mode exists? What is the goal of this scene?
...
7. **Pacing Type Verification:** Check the chapter's assigned pacing tag from `creative-plan.md` or the relevant `creative-plan/` shard. Is this an action chapter? Reflection? Transition? Write accordingly.
